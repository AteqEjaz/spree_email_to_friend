    ...
    respond_to do |format|
      format.html do
        ...
        if @mail_to_friend.valid? && captcha_passed
          flash[:notice] = Spree.t(:mail_sent_to, scope: :email_to_friend, email: @mail_to_friend.recipient_email).html_safe
          flash[:notice] << ActionController::Base.helpers.link_to(Spree.t(:send_to_other, scope: :email_to_friend), email_to_friend_path(@object.class.name.split("::").last.downcase, @object)).html_safe
